...
public record Rotor3(E0 e0, E1E2 e1e2, E2E3 e2e3, E3E1 e3e1) implements Geometric3<Rotor3> {
    ...
    public Rotor3(double e0, double e1e2, double e2e3, double e3e1) {
        this(new E0(e0), new E1E2(e1e2), new E2E3(e2e3), new E3E1(e3e1));
    }

    public DoubleVector toVector() {
        return Geometric3.simdVectorFrom(e0, e1e2, e2e3, e3e1);
    }
    ...
    public Vector3 rotate(Vector3 vector) {
        return this.times(vector).times(this.reverse()).vector();
...
//        return vec.times(rotorDagger).plus(trivec.times(rotorDagger)).vector();
    ...
    public Bivector3 rotate(Bivector3 bivector) {
        return this.times(bivector).times(this.reverse()).bivector();
    }

    public Rotor3 rotate(Rotor3 rotor3) {
        return this.times(rotor3).times(this.reverse());
    }
...
//                        .plus(e3e1.times(other.e2()))); bivector().inner(other)
    ...
    @Override
    public Rotor3 outer(Scalar other) {
